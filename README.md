...
このDirectoryはFramework Repository内のQuickstartであると同時に、Packagist Package `blackops/skeleton`のSource of Truthである。Release WorkflowがこのDirectoryだけを`kubotak-is/blackops-skeleton`へSplitし、Frameworkと同じVersionで公開する。Current Release SurfaceはExperimental Stable `1.1.0`であり、SkeletonはFramework `^1.1`を要求する。
